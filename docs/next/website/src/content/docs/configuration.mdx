...
Windows Terminal clients, including WSL profiles, use OSC 777 for `delivery = "terminal"`, but the outer terminal must contain [microsoft/terminal#20012](https://github.com/microsoft/terminal/pull/20012). As of July 15, 2026, Stable 1.24.11911.0 and Preview 1.25.1912.0 are unsupported; use a compatible Canary/development build or Windows Terminal 1.26 or newer once released, then enable `compatibility.allowOSC777`. See [Windows beta](/docs/windows-beta/#windows-terminal-notifications) for setup and security details.

## Bell

The Bell setting is a separate, default-off attention signal for the same background agent finished and needs-input events. It follows the notification delay and active-tab focus suppression, but does not require Toast or Sound to be enabled.
...
Herdr emits one ASCII BEL byte through the foreground client. The outer terminal and desktop decide the visible behavior:

- Windows Terminal can highlight or flash its taskbar icon. Set `bellStyle` to `"taskbar"` (or `"all"`) in the profile defaults:
  ...
  ```
- On macOS, Ghostty maps BEL to one Dock bounce through `bell-features = attention` while Ghostty is unfocused. `attention` is enabled by default and does not require macOS notification or Accessibility permission. Reload Ghostty with `Command+Shift+,` after editing its config. During `herdr --remote`, enable `[ui.bell]` in the local Mac client's config because that client emits BEL.
- On Linux, taskbar highlighting depends on the terminal emulator and desktop environment's visual-bell or urgency support.

Multiplexers such as tmux may intercept or suppress BEL, so their bell settings must also allow it to reach the outer terminal. Bell requests attention only; Herdr does not steal focus.
...
If you run [codex-trace](https://github.com/xianchoujiduluo/codex-trace) to read Agent conversations as turns and tool calls, Herdr can build a deep link to the conversation in a pane:
...
With this set, right-clicking an Agent pane adds **Copy session link**, which puts a URL like `http://127.0.0.1:1422/pi/01a0acf2-81bb-76f7-9e48-763112a909d3` on your clipboard. Paste it into a browser to open that conversation.

The action only appears when the pane has an Agent that reports a transcript path whose filename carries a recognizable session id. That covers pi, Claude Code, and Codex; other Agents and plain shells have no conversation to link to. Leaving `url` unset hides the action entirely.

Copying uses the same route as a mouse selection: OSC 52 when the terminal supports it, otherwise the platform clipboard tool. This is what lets the link reach your own clipboard when Herdr runs over SSH or in WSL.
