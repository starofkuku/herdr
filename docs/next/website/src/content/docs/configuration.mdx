...
`max_retries` defaults to `1`; set it to `0` (or `enabled = false`) to disable the automation, or `-1` to retry indefinitely. Infinite retries still use a one-second debounce and can repeat the same provider/model failure, so use that mode only when an external provider failover is expected.
