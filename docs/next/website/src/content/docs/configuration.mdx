...
Windows Terminal clients, including WSL profiles, use OSC 777 for `delivery = "terminal"`. Enable `compatibility.allowOSC777` in the Windows Terminal profile first; see [Windows beta](/docs/windows-beta/#windows-terminal-notifications) for setup and security details.
