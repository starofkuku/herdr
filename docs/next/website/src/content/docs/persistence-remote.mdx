...
For supported agent panes, `Alt+V` can bridge an image from the local clipboard. The remote server first verifies the detected agent and its image-paste transport; only then does the local client read the clipboard and copy the image to a private temporary file on the remote host. Herdr pastes the absolute path for path-aware agents and an `@path` mention for Amp. If the pane is unsupported or has not been detected yet, Herdr forwards the original key without reading the clipboard.

| Remote agent | Image bridge |
| --- | --- |
| Pi, Claude Code, Codex, Cursor CLI, Cline, OMP, MastraCode, OpenCode, GitHub Copilot CLI, Kiro, Factory Droid, Grok, Hermes, Kilo Code, Qoder CLI, Maki | Staged absolute image path |
| Amp | Staged `@path` mention |
