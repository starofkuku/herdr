...
herdr update web              # download and install the newest web UI page
herdr update web --check      # report the available web UI version, writing nothing
herdr switch 0.7.30            # install a specific released version
herdr switch --list            # list released versions, marking the installed one
...
Serve the browser UI for every session. Requires `HERDR_WEB_KEY`; see [Web UI](/docs/web-ui/).
...
```bash
herdr pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--offset N]
herdr pane read <pane_id> --source visible --ansi
herdr pane read <pane_id> --source recent-unwrapped --lines 120
herdr pane read <pane_id> --source recent-unwrapped --lines 120 --offset 120
```

`--lines` defaults to 80 and is capped at 2000 per call. `--offset` skips that many rows back from the newest row, so repeated calls walk the retained history a page at a time. An offset past the oldest retained row returns empty output.
...
Publish a structured question an agent is waiting on, and collect the answer:

```bash
herdr pane report-interaction <pane_id> \
  --source ID \
  --request-id ID \
  ...
  [--option QUESTION_ID=OPTION_ID=LABEL] \
  [--option-description OPTION_ID=TEXT] \
  [--multi-select QUESTION_ID] \
  [--allow-custom QUESTION_ID] \
  [--seq N] \
  [--ttl-ms N]

herdr pane clear-interaction <pane_id> --source ID --request-id ID [--seq N]

herdr pane answer-interaction <pane_id> --request-id ID \
  --answer QUESTION_ID=OPTION_ID[,OPTION_ID...]

herdr pane take-interaction-answer <pane_id> --source ID --request-id ID
```

Use this when the agent publishes its own question and options, so a client can present the options the agent actually offered instead of leaving the reader to work them out from the rendered screen. The question and option ids are passed through unchanged, so the integration maps them back to its own protocol.

`clear-interaction` withdraws a request that is no longer pending, and only clears the request named by `--request-id`, so a client cannot clear a newer question that replaced the one it saw.

`take-interaction-answer` is how the integration that asked collects the answer. It prints one JSON object:

```json
{"answers": [{"question_id": "decision", "option_ids": ["allow"]}], "pending": false}
```

`pending` reports whether the request is still outstanding. An integration that waits for an answer polls this command and keeps waiting while `pending` is `true`; a `false` with no answers means the question was answered elsewhere, withdrawn, or expired, so the wait should end and the decision be handed back rather than held open.
...
`agent restart` requires the target Agent to be idle and to have an officially reported native session. It schedules the Agent to resume in the same pane and preserves the existing layout; the command returns after scheduling and does not wait for the resumed Agent to become ready.
...
herdr integration uninstall zcode
