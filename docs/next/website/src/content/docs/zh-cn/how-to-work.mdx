...
想让远程会话用起来像本地时,用这条路径。对于远程识别到的 Codex pane,macOS、Linux 或 WSL 本地客户端可以处理 `Alt+V`: 服务器授权后才读取本地剪贴板图像,并把它暂存到远程主机供 Codex 附加。其他 pane 会收到原始按键。不支持原生 Windows Herdr 客户端。如果你先 SSH 再在服务器上运行 `herdr`,Herdr 就完全运行在那台服务器上,无法读取你本地桌面的剪贴板。
...
本地工作用 `herdr`。想让 Herdr 在远程 shell 上表现得像 tmux,或者在用手机 SSH 客户端时,用 `ssh you@server` 再 `herdr`。想要一个连接远程会话的本地瘦客户端 (包括服务器授权的 Codex 剪贴板图像粘贴) 时,用 `herdr --remote <host>`。
