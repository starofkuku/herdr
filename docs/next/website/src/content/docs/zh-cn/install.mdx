...
`herdr switch` 会用指定版本就地替换当前正在运行的版本：

```bash
herdr switch 0.7.30    # 比当前已安装的版本更旧
herdr switch --list    # 列出可用版本，并标出已安装的那个
```

仅适用于直接安装。与 `herdr update` 相同的理由，它在 Homebrew、mise、Nix 下会拒绝执行：这些工具自己管理二进制文件。

下载会用 GitHub 为该发布资产公布的 sha256 校验和进行校验。早于该元数据存在的发布没有校验和，此时命令会说明情况而不是直接失败——否则那些版本就无法使用了。

正在运行的会话与更新时同样处理，包括在客户端/服务端协议变化时询问是否停止旧服务端。
