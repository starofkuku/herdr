...
对于 Codex pane,`Alt+V` 可以桥接本地剪贴板中的图像。远程服务器会先确认当前聚焦 pane 已被识别为 Codex;只有确认后,本地客户端才会读取剪贴板,把图像复制到远程临时文件,并粘贴远程路径,让 Codex 创建 `[Image #1]` 附件。如果当前 pane 不是 Codex 或尚未识别,Herdr 会原样转发按键,不会读取剪贴板。本地剪贴板读取支持 macOS、Linux 和运行在 WSL 内的 Herdr,不支持原生 Windows Herdr 客户端。先 SSH 到服务器再启动 Herdr 的方式无法使用此功能,因为远程进程访问不到本地桌面剪贴板。

在 macOS 上,终端通常会自行拦截 `Command+V`,而单独的 `Option+V` 可能被转换为组合字符,而不是 Alt 组合键。建议在 Mac 本地配置 `keys.remote_image_paste = "ctrl+alt+v"`;修改后请分离并重新连接,再按 Control+Option+V。裸 `Ctrl+V` 是 Codex 自己的剪贴板快捷键,因此在无桌面的 Linux 主机上按它会绕过 Herdr,并可能出现 X11 剪贴板不可用或连接超时错误。
