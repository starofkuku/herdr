...
对于受支持的 Agent pane，`Alt+V` 可以桥接本地剪贴板中的图像。远程服务器会先确认当前识别到的 Agent 及其图片粘贴方式；只有确认支持后，本地客户端才会读取剪贴板，并把图像复制到远程主机上的私有临时文件。对于支持路径的 Agent，Herdr 会粘贴绝对路径；对于 Amp，则会粘贴 `@路径`。如果当前 Agent 不受支持或尚未识别，Herdr 会原样转发按键，不会读取剪贴板。

| 远程 Agent | 图片桥接方式 |
| --- | --- |
| Pi、Claude Code、Codex、Cursor CLI、Cline、OMP、MastraCode、OpenCode、GitHub Copilot CLI、Kiro、Factory Droid、Grok、Hermes、Kilo Code、Qoder CLI、Maki | 远程暂存图片的绝对路径 |
| Amp | 远程暂存图片的 `@路径` |
