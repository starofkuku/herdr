...
herdr update web              # 下载并安装最新的 Web UI 页面
herdr update web --check      # 只报告可用的 Web UI 版本，不写入任何内容
...
为所有会话提供浏览器 UI。需要 `HERDR_WEB_KEY`;详见 [Web UI](/zh-cn/docs/web-ui/)。
...
`--lines` 默认 80，单次调用上限 2000 行。`--offset` 从最新行往前跳过指定行数，因此重复调用可以逐页回溯保留的历史。偏移超过最旧的保留行时返回空输出。
...
当 Agent 自己发布问题与选项时使用。这样客户端可以展示 Agent 实际提供的选项,而不必让读者从渲染画面中自行推断。问题与选项 id 会原样传递,以便集成映射回自己的协议。

`clear-interaction` 用于撤回一个不再挂起的问题,并且只清除 `--request-id` 指定的那一个,因此客户端不会误删替换掉它所见到内容的新问题。

`take-interaction-answer` 是提问的集成回收作答的方式。它输出一个 JSON 对象:

```json
{"answers": [{"question_id": "decision", "option_ids": ["allow"]}], "pending": false}
```

`pending` 表示该请求是否仍然挂起。等待作答的集成会轮询此命令,并在 `pending` 为 `true` 时继续等待;答案为空且为 `false` 时,表示已在别处作答、已被撤回或已过期,此时应结束等待并把决定交回调用方。
...
`agent restart` 要求目标智能体处于空闲状态,并且已通过官方集成上报可恢复的原生会话。它会在同一窗格中安排恢复并保留现有布局;命令在安排完成后返回,不会等待恢复后的智能体进入就绪状态。
