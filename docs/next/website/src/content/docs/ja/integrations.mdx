---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode、Grok Build、ZCode 向けの Herdr インテグレーションをインストールします。
---

Herdr は対応エージェントを自動的に検出します。公式インテグレーションは、復元のためのネイティブセッション識別、ライフサイクル状態の報告、またはその両方を追加できます。

Claude Code/Codex/Copilot/Devin 系のフックによるエージェントネイティブのセッション復元、Pi/OMP/Kimi/OpenCode/Kilo/Hermes/Grok 系のフックまたはプラグインによる直接のライフサイクル報告、あるいはその両方が欲しいときにインテグレーションを使ってください。状態権威モデルの全体像は[エージェント](/ja/docs/agents/)を参照してください。
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、MastraCode、Grok Build | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
ZCode は `~/.claude/settings.json` ではなく、独自の `config.json` からフックを読み込みます。`~/.claude/settings.json` は読み取り専用のレガシー互換として一覧表示されるだけです。Herdr は 2 つのフックを登録します。`SessionStart` はネイティブセッション ID(`sess_...`)を記録し、サーバー再起動後に `zcode --resume <id>` で復元できるようにします。`PermissionRequest` はターミナルが独自のプロンプトを表示する前に、Web UI から承認に応答できるようにします。エージェントの状態は Claude Code 統合と同様に画面検出に委ねます。

利用できるのはユーザースコープのフックだけです。ZCode はプロジェクトフックをワークスペース信頼レビューの背後に置いていますが、その機能は CLI や TUI では有効になっていないため、`~/.zcode/cli/config.json` 以外にインストールしたフックは実行されません。アンインストールすると、管理対象のフックスクリプトとフックエントリが削除され、`config.json` の残りの内容は変更されません。

## Grok Build

Grok Build インテグレーションをインストールします:
...
`GROK_HOME` が設定されている場合は `$GROK_HOME/hooks/` を使います。hooks ディレクトリが無い場合は作成します。インテグレーションはライフサイクル状態とネイティブセッション id（`GROK_SESSION_ID` / フック payload の `sessionId`）を報告し、サーバー再起動後に `grok --resume <id>` で resume できます。サブエージェント完了はルートを idle にしません。ルートの `Stop` / `SessionEnd` が idle を担当します。アンインストールは管理対象のフックスクリプトと JSON を削除します。
...
[`@juicesharp/rpiv-ask-user-question`](https://www.npmjs.com/package/@juicesharp/rpiv-ask-user-question) プラグインが入っている場合、この拡張はそのアンケートを Web UI にも反映します。プラグインは待っている質問を自身の文書化されたイベント契約で公開するため、Web UI は画面を読み取るのではなく、エージェントが実際に作成した選択肢を提示できます。また、質問中は画面テキストに頼らず正確に `blocked` を報告します。

回答はプラグイン自身のダイアログを操作して届けます。選択した行までカーソルを移動し、`Enter` を押します。ダイアログは表示されたままでターミナルからも回答できるため、どちらの経路も使え、どちらかが唯一の正解ではありません。

2 つの制限は意図的です。ダイアログには数字キーがありません（反応するのは矢印、`Enter`、`Esc`、`Tab`、`Space` と一部のコントロールキーのみ）ので、選択は数字を押すのではなく移動して確定します。また、反映されるのは質問が 1 つのダイアログだけです。複数質問のダイアログ（タブ、チェックボックス、質問ごとのカスタム回答）には忠実なキー列が存在しないため、近似せずターミナルに任せます。カスタム回答の入力も同様にターミナルで行います。

Herdr は `HERDR_PI_ASK_WAIT_MS`（既定 120 秒）の間、質問を提示し続けます。それまでに誰も回答しなければパネルは取り下げられ、ダイアログはそのまま残ります。
...
`PermissionRequest` フックも登録され、承認を Web UI から回答できるようになります。Claude がツール呼び出しについて確認しようとすると、このフックはその承認を構造化された質問として公開し、数秒間回答を待ち、回答が届けば Claude 自身の allow/deny の判断として返します。時間内に誰も回答しなければフックは何も判断を返さず、Claude は通常のプロンプトを表示します。したがってターミナルの利用者が待たされるのはその短い猶予の分だけです。待ち時間はエージェントの環境変数 `HERDR_CLAUDE_PERMISSION_WAIT_MS` で変更できます。Web UI で回答されなかった承認の動作は、このフックが無かったときとまったく同じです。
...
同時に `PermissionRequest` フックも登録され、承認を Web UI から回答できるようになります。Codex がコマンド、ファイル変更、権限について確認しようとすると、このフックはその承認を構造化された質問として公開し、数秒間回答を待ち、回答が届けば Codex 自身の allow/deny の判断として返します。時間内に誰も回答しなければフックは何も判断を返さず、Codex は通常のプロンプトを表示します。したがってターミナルの利用者が待たされるのはその短い猶予の分だけです。待ち時間はエージェントの環境変数 `HERDR_CODEX_PERMISSION_WAIT_MS` で変更できます。Web UI で回答されなかった承認の動作は、このフックが無かったときとまったく同じです。

他の Codex フックと同様、実行には信頼の承認が必要です。Codex はフックを最初に読み込むときに確認を求め、拒否するとフックは無効のまま、それ以外のインテグレーションは動作します。
...
Linux と macOS では、同じインストールによって Herdr サーバーホスト上の
ファーストパーティ Codex rollout モニタープラグインも有効になります。公式の
Codex セッション id に対応する rollout JSONL を読み、構造化されたアクティビティ
診断を報告します。`continue` の送信、Escape、コマンドの中断、モデル変更は行わず、
Herdr の `working`、`idle`、`blocked` 状態も変更しません。診断があるエージェント
ペインを右クリックして **Inspect activity** を選ぶと、右上に閉じられるカードが
表示されます。

設定はサーバーホストにあります。ディレクトリは次のコマンドで確認できます:

```bash
herdr plugin config-dir herdr.codex-rollout-monitor
```

既定の `config.toml` は次のとおりです:
...
`suspected_stalled_after_seconds` は、アクティブなツールに新しい rollout イベントが
ない場合の警告しきい値です。`silent_after_seconds` は途中の無出力コマンド状態だけを
制御します。`notification.enabled` は疑わしい停止のエピソードごとに最大一度の通知を
制御し、その後に進行があればエピソードを閉じます。`monitor.enabled` はプラグイン
サービス全体を無効にせずに監視を一時停止します。設定ファイルは実行中に再読み込み
されます。ネイティブ Windows では Codex フックのみをインストールし、この監視
サービスは実行しません。
