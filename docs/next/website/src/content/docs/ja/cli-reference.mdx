...
herdr update web              # 最新の Web UI ページをダウンロードしてインストール
herdr update web --check      # 何も書き込まず、利用可能な Web UI バージョンを報告
...
すべてのセッション向けにブラウザ UI を配信します。`HERDR_WEB_KEY` が必要です。詳細は [Web UI](/docs/web-ui/) を参照してください。
...
`--lines` の既定値は 80 で、1 回の呼び出しで最大 2000 行です。`--offset` は最新行から数えてその行数分だけ遡るため、繰り返し呼ぶことで保持されている履歴を 1 ページずつ辿れます。保持されている最古の行より前を指定すると空の出力になります。
...
エージェントが待っている構造化された質問を公開し、回答を回収する:
...
エージェントが自身の質問と選択肢を公開している場合に使います。これにより、クライアントは描画された画面から読み取るのではなく、エージェントが実際に提示した選択肢を表示できます。質問と選択肢の id はそのまま渡されるため、インテグレーションが自身のプロトコルへ対応付けられます。

`clear-interaction` はすでに保留でなくなったリクエストを取り下げます。`--request-id` で指定したリクエストだけを消すため、クライアントが見たものを置き換えた新しい質問を誤って消すことはありません。

`take-interaction-answer` は、質問を出したインテグレーションが回答を回収する方法です。次の JSON オブジェクトを 1 つ出力します:

```json
{"answers": [{"question_id": "decision", "option_ids": ["allow"]}], "pending": false}
```

`pending` はそのリクエストがまだ保留中かどうかを示します。回答を待つインテグレーションはこのコマンドをポーリングし、`pending` が `true` の間は待ち続けます。回答が空で `false` の場合は、別の場所で回答されたか、取り下げられたか、期限切れであることを意味するため、待機を終了して判断を呼び出し元に返します。
