...
Windows Terminal は特定のエージェント向けに画像貼り付け経路をサポートしているかもしれませんが、Herdr 自身のクリップボード画像リーダーはネイティブ Windows クライアントにはまだ配線されていません。ネイティブ Windows の `Alt+V` 画像貼り付けは未検証のままです。これは WSL 内で Herdr を動かす場合とは別で、WSL のローカルクライアントは Windows PowerShell を通じて Codex 専用の `herdr --remote` 画像ブリッジを利用できます。
