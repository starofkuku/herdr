...
WSL プロファイルを含む Windows Terminal クライアントでは、`delivery = "terminal"` に OSC 777 を使用します。先に Windows Terminal プロファイルで `compatibility.allowOSC777` を有効にしてください。設定方法とセキュリティ上の注意は [Windows ベータ](/docs/windows-beta/#windows-terminal-通知)を参照してください。
