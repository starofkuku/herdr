...
リモートセッションをローカルのように感じたいときはこの方法を使ってください。リモートで Codex として検出された pane では、macOS、Linux、または WSL のローカルクライアントが `Alt+V` を処理し、サーバーの許可後にだけローカルクリップボード画像を読み、Codex が添付できるようリモートホストへ一時保存します。ほかの pane には元のキーが届きます。ネイティブ Windows の Herdr クライアントには対応しません。先に SSH してサーバー上で `herdr` を実行した場合、Herdr は完全にそのサーバー上で動くため、ローカルデスクトップのクリップボードは読めません。
...
ローカル作業には `herdr` を使います。リモートシェル上で Herdr を tmux のように使いたいときや、スマートフォンの SSH クライアントを使っているときは `ssh you@server` してから `herdr` を使います。リモートセッション用のローカルシンクライアントが欲しいとき (サーバー許可付きの Codex クリップボード画像貼り付けを含む) は `herdr --remote <host>` を使います。
