---
title: Plugins
description: Author local Herdr plugins with manifest actions, event hooks, panes, and managed services.
---
...
plugin v1. Actions, event hooks, panes, managed services, and link handlers are all declared in
the manifest.
...
Action ids, pane ids, service ids, and link handler ids are local ids inside the plugin.
...
Actions that declare `contexts = ["pane"]` are also available from an Agent
pane's right-click context menu. Herdr shows enabled pane actions when the
target pane is recognized as an Agent, then invokes the selected action with
that pane as the focused plugin context. Actions remain available through the
CLI and keybindings as well.

Use `platforms = ["linux", "macos", "windows"]` to declare where the plugin
can run. Build commands, actions, event hooks, panes, services, and link handlers can
...
Use `[[services]]` for a long-running background process such as a monitor or
bridge. Herdr starts one process for each declared service of an enabled plugin
on the server host. Services receive the normal plugin environment, including
`HERDR_PLUGIN_ROOT`, `HERDR_PLUGIN_CONFIG_DIR`, `HERDR_PLUGIN_STATE_DIR`,
`HERDR_BIN_PATH`, `HERDR_SOCKET_PATH`, `HERDR_PLUGIN_ID`, and
`HERDR_PLUGIN_SERVICE_ID`.

`restart = "always"` is the default. When such a service exits or cannot start,
Herdr retries with exponential backoff from one second to 30 seconds. Use
`restart = "never"` for a one-shot helper that must not restart. Services stop
when their plugin is disabled or unlinked, during a live server handoff, and
when the server shuts down. They are not action invocations: they do not consume
the one-shot plugin command limit and their standard streams are not captured as
plugin command logs.

Service-level `platforms` works like the other manifest entrypoints and
overrides the top-level platform list. `herdr plugin install` includes declared
service commands in its install preview. A service is ordinary trusted plugin
code, so treat an always-running service as part of the plugin's security
review.
