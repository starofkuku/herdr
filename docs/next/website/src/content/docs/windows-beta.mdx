...
Herdr can send agent notifications from native Windows sessions and WSL profiles through Windows Terminal's OSC 777 support. In Windows Terminal `settings.json`, enable the compatibility setting for the profile that launches Herdr:
