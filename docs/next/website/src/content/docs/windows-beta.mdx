...
Windows Terminal may support image paste paths for specific agents, but Herdr's own clipboard-image reader is not wired for the native Windows client. Native Windows `Alt+V` image paste remains unverified. This is separate from running Herdr inside WSL: a WSL-based local client can use the supported-agent `herdr --remote` image bridge through Windows PowerShell.
