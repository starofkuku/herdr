...
if (-not [string]::IsNullOrWhiteSpace($hookEventName) -and $hookEventName -ne "SessionStart" -and $hookEventName -ne "PermissionRequest") { exit 0 }
...
if ($Action -eq "permission") {
    ...
    # reader is judging.
    $summary = ""
    foreach ($key in @("command", "cmd", "path", "file_path", "url", "query")) {
    ...
    $waitMs = 3000
    if (-not [string]::IsNullOrWhiteSpace($env:HERDR_CODEX_PERMISSION_WAIT_MS)) {
        $parsed = 0
        if ([int]::TryParse($env:HERDR_CODEX_PERMISSION_WAIT_MS, [ref]$parsed)) { $waitMs = $parsed }
    }
    ...
    if ($null -eq $decision) {
        # No decision: exit with no output so Codex draws its own prompt.
        exit 0
    }
    # The decision must be nested under hookSpecificOutput, with behavior inside
    # decision. A top-level behavior is rejected by Codex's schema and would
    # silently do nothing.
    $hookSpecific = @{ hookEventName = "PermissionRequest"; decision = @{ behavior = $decision } }
    if ($decision -eq "deny") { $hookSpecific.decision.message = "Denied in the Herdr web UI." }
    Write-Output (@{ hookSpecificOutput = $hookSpecific } | ConvertTo-Json -Depth 6 -Compress)
    exit 0
}
