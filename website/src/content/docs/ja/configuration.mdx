...
Codex の画面に正確な容量エラー `Selected model is at capacity. Please try a different model.` が表示されたとき、Herdr はエラーを報告した同じ pane に `继续` と Enter を自動送信できます。
...
`max_retries` のデフォルトは `1` です。`0`（または `enabled = false`）で無効化し、`-1` で無期限リトライにできます。無期限リトライにも 1 秒のデバウンスがあり、同じ provider/model の容量エラーを繰り返す可能性があるため、外部のフェイルオーバーが期待できる場合だけ使用してください。
