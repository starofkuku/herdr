...
| ペイン | `pane.split`、`pane.swap`、`pane.move`、`pane.zoom`、`pane.layout`、`pane.process_info`、`pane.neighbor`、`pane.edges`、`pane.focus_direction`、`pane.resize`、`pane.list`、`pane.current`、`pane.get`、`pane.rename`、`pane.send_text`、`pane.send_keys`、`pane.send_input`、`pane.read`、`pane.report_agent`、`pane.report_agent_session`、`pane.report_metadata`、`pane.report_diagnostic`、`pane.clear_diagnostic`、`pane.report_interaction`、`pane.clear_interaction`、`pane.answer_interaction`、`pane.take_interaction_answer`、`pane.clear_agent_authority`、`pane.release_agent`、`pane.close`、`pane.wait_for_output` |
...
`pane.report_diagnostic` は、エージェントのライフサイクル状態を引き継がない
構造化された短寿命の診断に使います。`pane.get`、`pane.list`、`agent.get`、
`agent.list` は、有効な診断を `diagnostics` 配列で返します。診断はサーバーの
ランタイムだけに保持され、コールドリスタート後には復元されません。
...
1 ペインにつき最大 8 件、各診断は最大 12 フィールドです。`ttl_ms` は省略可能で、
メタデータと同じ 1 から 86400000 ミリ秒の範囲です。`seq` を使った後は、同じ
source と diagnostic id に対する報告と `pane.clear_diagnostic` で、厳密に大きい
シーケンス番号が必要です。シーケンスなしの報告は、シーケンス付きの記録を置き換えません。
