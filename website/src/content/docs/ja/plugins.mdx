---
title: プラグイン
description: マニフェストのアクション、イベントフック、ペイン、管理サービスを備えたローカル Herdr プラグインを作成します。
---
...
プラグイン v1 の範囲外です。アクション、イベントフック、ペイン、管理サービス、リンクハンドラーは
すべてマニフェストで宣言します。
...
アクション id、ペイン id、サービス id、リンクハンドラー id はプラグイン内部のローカル id です。
...
ビルドコマンド、アクション、イベントフック、ペイン、サービス、リンクハンドラーも独自の
...
`[[services]]` は、モニターやブリッジのような長時間動作するバックグラウンド
プロセスに使います。Herdr は有効なプラグインの各サービスをサーバーホスト上で
1 プロセスずつ起動します。サービスには `HERDR_PLUGIN_ROOT`、
`HERDR_PLUGIN_CONFIG_DIR`、`HERDR_PLUGIN_STATE_DIR`、`HERDR_BIN_PATH`、
`HERDR_SOCKET_PATH`、`HERDR_PLUGIN_ID`、`HERDR_PLUGIN_SERVICE_ID` を含む通常の
プラグイン環境が渡されます。

既定値は `restart = "always"` です。終了または起動失敗後は 1 秒から 30 秒までの
指数バックオフで再試行します。再起動しない場合は `restart = "never"` を使います。
プラグインの無効化・リンク解除、ライブサーバーハンドオフ、サーバー終了時には
サービスも停止します。サービスは一回限りのプラグインコマンド枠を消費せず、標準
出力と標準エラーはプラグインコマンドログには記録されません。

サービス独自の `platforms` はトップレベルの指定を上書きします。
`herdr plugin install` のプレビューにはサービスコマンドも表示されます。常駐サービスも
通常の信頼済みプラグインコードなので、インストール前にコマンドを確認してください。
