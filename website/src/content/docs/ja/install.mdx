...
`herdr switch` は指定したリリース版を、いま実行しているものと入れ替えてインストールします:

```bash
herdr switch 0.7.30    # インストール済みより古いバージョン
herdr switch --list    # 利用できるバージョン一覧（インストール済みに印が付きます）
```

直接インストールした場合のみ使えます。Homebrew、mise、Nix では `herdr update` と同じ理由で拒否します。これらはバイナリを自分で管理するためです。

ダウンロードは、リリースのアセットに対して GitHub が公開している sha256 チェックサムで検証します。そのメタデータが存在しなかった頃のリリースにはチェックサムがないため、失敗させるのではなくその旨を表示します。そうしなければ、それらのバージョンに到達できなくなるからです。

実行中のセッションはアップデートと同じように扱われ、クライアント/サーバーのプロトコルが変わった場合に古いサーバーを停止するかどうかの確認も含まれます。
