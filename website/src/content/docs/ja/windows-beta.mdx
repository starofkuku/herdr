...
Herdr は Windows Terminal の OSC 777 サポートを使い、Windows ネイティブセッションと WSL プロファイルからエージェント通知を送信できます。Windows Terminal の `settings.json` で、Herdr を起動するプロファイルの互換性設定を有効にします:
...
最近の Windows Terminal では、プロファイル UI に **Allow OSC 777 (Desktop Notification) to show toast notifications** と表示されます。この設定が見つからない場合は Windows Terminal を更新してください。OSC 777 を有効にすると、ターミナルアプリケーションが任意のテキストを含む Windows 通知を作成できるため、実行するプログラムを信頼できるプロファイルでのみ有効にしてください。
