...
Codex pane では、`Alt+V` でローカルクリップボードの画像をブリッジできます。リモートサーバーは、フォーカス中の pane が Codex として検出されていることを先に確認します。確認できた場合にのみローカルクライアントがクリップボードを読み、画像をリモートの一時ファイルへコピーし、そのパスを貼り付けて Codex に `[Image #1]` 添付を作成させます。Codex でない場合や未検出の場合は、クリップボードを読まず元のキーをそのまま転送します。ローカルのクリップボード読み取りは macOS、Linux、WSL 内で動く Herdr に対応し、ネイティブ Windows の Herdr クライアントには対応しません。先に SSH してサーバー上で Herdr を起動する方法では、ローカルデスクトップのクリップボードへアクセスできないため利用できません。

macOS では通常、ターミナル自身が `Command+V` を処理し、単独の `Option+V` は Alt コードではなく合成文字へ変換されることがあります。Mac 側のローカル設定には `keys.remote_image_paste = "ctrl+alt+v"` を推奨します。変更後はデタッチして再接続し、Control+Option+V を押してください。素の `Ctrl+V` は Codex 自身のクリップボードショートカットなので、GUI のない Linux ホスト上の Codex へ送ると Herdr を迂回し、X11 クリップボードが利用できない、または接続がタイムアウトしたというエラーになることがあります。
