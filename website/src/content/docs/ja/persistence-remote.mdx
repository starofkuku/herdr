...
対応 Agent の pane では、`Alt+V` でローカルクリップボードの画像をブリッジできます。リモートサーバーは、検出された Agent と画像貼り付け方式を先に確認します。対応している場合にのみローカルクライアントがクリップボードを読み、画像をリモートホスト上の非公開一時ファイルへコピーします。パス対応 Agent には絶対パスを、Amp には `@path` メンションを貼り付けます。未対応または未検出の場合は、クリップボードを読まず元のキーをそのまま転送します。

| リモート Agent | 画像ブリッジ |
| --- | --- |
| Pi、Claude Code、Codex、Cursor CLI、Cline、OMP、MastraCode、OpenCode、GitHub Copilot CLI、Kiro、Factory Droid、Grok、Hermes、Kilo Code、Qoder CLI、Maki | 一時保存した画像の絶対パス |
| Amp | 一時保存した画像の `@path` メンション |
