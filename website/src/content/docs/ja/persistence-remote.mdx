...
このモードでは、ローカルの Herdr はシンクライアントです。SSH 越しに接続し、リモートの Herdr サーバーを起動またはアタッチして、UI をローカルターミナルにストリーミングします。

対応 Agent の pane では、`Alt+V` でローカルクリップボードの画像をブリッジできます。リモートサーバーは、検出された Agent と画像貼り付け方式を先に確認します。対応している場合にのみローカルクライアントがクリップボードを読み、画像をリモートホスト上の非公開一時ファイルへコピーします。パス対応 Agent には絶対パスを、Pi と Amp には `@path` メンションを貼り付けます。未対応または未検出の場合は、クリップボードを読まず元のキーをそのまま転送します。

| リモート Agent | 画像ブリッジ |
| --- | --- |
| Claude Code、Codex、Cursor CLI、Cline、OMP、MastraCode、OpenCode、GitHub Copilot CLI、Kiro、Factory Droid、Grok、Hermes、Kilo Code、Qoder CLI、Maki、ZCode | 一時保存した画像の絶対パス |
| Pi、Amp | 一時保存した画像の `@path` メンション |
| Gemini CLI | 未対応: 通常の Herdr 一時パスは Gemini が許可するプロジェクトディレクトリ外 |
| Kimi CLI | 未対応: ネイティブ画像貼り付けが Kimi の非公開 prompt-cache token 形式に依存 |
| Devin CLI、Antigravity | 未対応: 安定した対話型 CLI の画像パス契約が未公開 |

ローカルのクリップボード読み取りは macOS、Linux、WSL 内で動く Herdr に対応し、ネイティブ Windows の Herdr クライアントには対応しません。先に SSH してサーバー上で Herdr を起動する方法では、ローカルデスクトップのクリップボードへアクセスできないため利用できません。

macOS では通常、ターミナル自身が `Command+V` を処理し、単独の `Option+V` は Alt コードではなく合成文字へ変換されることがあります。Mac 側のローカル設定には `keys.remote_image_paste = "ctrl+alt+v"` を推奨します。変更後はデタッチして再接続し、Control+Option+V を押してください。素の `Ctrl+V` は Codex などの Agent 自身のクリップボードショートカットなので、GUI のない Linux ホスト上の Agent へ送ると Herdr を迂回し、X11 クリップボードが利用できない、または接続がタイムアウトしたというエラーになることがあります。
...
ローカルとリモートのプラットフォームが一致する場合、直接インストールでは Herdr は現在のローカルバイナリをコピーできます。Homebrew、mise、Nix のインストールの場合、またはプラットフォームが異なる場合、この fork は `https://raw.githubusercontent.com/starofkuku/herdr/master/website/latest.json` から現在のクライアントバージョンに一致するリリースアセットをダウンロードします。
