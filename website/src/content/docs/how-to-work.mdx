...
Use this path when you want the remote session to feel local. For a remotely detected agent with a supported path-based image attachment interface, the local macOS, Linux, or WSL client can handle `Alt+V`, read the local clipboard image only after server authorization, and stage it on the remote host for the agent to attach. Unsupported or undetected panes receive the original key. Native Windows Herdr clients are not supported. If you SSH first and run `herdr` on the server, Herdr runs entirely on that server and cannot read your local desktop clipboard.
