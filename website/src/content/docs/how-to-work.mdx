...
Use this path when you want the remote session to feel local. For a remotely detected Codex pane, the local macOS, Linux, or WSL client can handle `Alt+V`, read the local clipboard image only after server authorization, and stage it on the remote host for Codex to attach. Other panes receive the original key. Native Windows Herdr clients are not supported. If you SSH first and run `herdr` on the server, Herdr runs entirely on that server and cannot read your local desktop clipboard.
...
Use `herdr` for local work. Use `ssh you@server` then `herdr` when you want Herdr to behave like tmux on that remote shell or when you are using a phone SSH client. Use `herdr --remote <host>` when you want a local thin client for a remote session, including server-authorized Codex clipboard image paste.
