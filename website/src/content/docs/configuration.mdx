...
Herdr can automatically retry Codex when its screen shows the exact capacity message `Selected model is at capacity. Please try a different model.` It sends `继续` followed by Enter to the same pane that reported the error.
...
`max_retries` defaults to `1`; set it to `0` (or `enabled = false`) to disable the automation, or `-1` to retry indefinitely. Each visible capacity-error occurrence is consumed at most once, so a stable screen refresh cannot resend `继续`; a later upstream response with the same error is treated as a new occurrence.
