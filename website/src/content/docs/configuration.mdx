...
- Windows Terminal can highlight or flash its taskbar icon. Set `bellStyle` to `"taskbar"` (or `"all"`) in the profile defaults:
  ...
  ```
- On macOS, Ghostty and other terminals can map BEL to a Dock bounce when their notification/attention setting is enabled and macOS grants the required permission.
