---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, MastraCode, Grok Build, and ZCode.
---
...
Use integrations when you want native agent session restore from Claude Code/Codex/Copilot/Devin-style hooks, direct lifecycle reports from Pi/OMP/Kimi/OpenCode/Kilo/Hermes/Grok-style hooks or plugins, or both. See [Agents](/docs/agents/) for the full status authority model.
...
Native session restore requires current Herdr integrations: Pi integration version `2`, OMP version `3`, Claude Code version `6`, Codex version `7`, GitHub Copilot CLI version `2`, Devin CLI version `2`, Droid version `2`, Kimi Code CLI version `3`, Qoder CLI version `2`, Cursor Agent CLI version `1`, OpenCode version `5`, Kilo Code CLI version `1`, Hermes Agent version `2`, MastraCode version `1`, or Grok Build version `2`. Check installed versions with `herdr integration status`.

## ZCode

Install the ZCode integration:

```bash
herdr integration install zcode
```

Herdr writes:

```text
~/.zcode/cli/hooks/herdr-agent-state.sh
~/.zcode/cli/config.json
```

ZCode reads hooks from its own `config.json`, not from `~/.claude/settings.json`, which it only lists as read-only legacy compatibility. Herdr registers two hooks there: `SessionStart` records the native session id (`sess_...`) so Herdr can resume with `zcode --resume <id>` after a server restart, and `PermissionRequest` lets the web UI answer an approval before the terminal draws its own prompt. Agent state is left to screen detection, matching the Claude Code integration.

Only user-scope hooks are usable. ZCode gates project hooks behind a workspace trust review that is not wired up in its CLI or TUI, so a hook installed outside `~/.zcode/cli/config.json` never runs there. Uninstall removes the managed hook script and the hook entries, leaving the rest of `config.json` untouched.
...
~/.grok/hooks/herdr-agent-state.json
```

If `GROK_HOME` is set, Herdr uses `$GROK_HOME/hooks/` instead. The hooks directory is created when missing. The integration reports lifecycle state and the native session id (`GROK_SESSION_ID` / hook payload `sessionId`) so Herdr can resume with `grok --resume <id>` after a server restart. Subagent completion does not force root idle; root `Stop` / `SessionEnd` still do. Uninstall removes the managed hook script and JSON file.
...
With the [`@juicesharp/rpiv-ask-user-question`](https://www.npmjs.com/package/@juicesharp/rpiv-ask-user-question) plugin installed, the extension also mirrors that plugin's questionnaire into the web UI. The plugin publishes the question it is waiting on through its documented event contract, so the web UI can offer the options the agent actually authored rather than a screen scrape, and a question also reports `blocked` precisely instead of relying on screen text.

Answering is delivered by driving the plugin's own dialog: the extension moves the cursor to the chosen row and presses `Enter`. The dialog stays on screen and remains answerable in the terminal, so both routes work and neither is authoritative.

Two limits are deliberate. The dialog has no number keys — it reacts only to arrows, `Enter`, `Esc`, `Tab`, `Space`, and a few control chords — so a choice is made by moving and confirming rather than by pressing a digit. And only a single-question dialog is mirrored: a multi-question dialog (tabs, checkboxes, per-question custom answers) has no faithful key sequence, so it is left entirely to the terminal rather than answered approximately. Typing a custom answer is likewise left to the terminal.

Herdr keeps offering the question for `HERDR_PI_ASK_WAIT_MS` (120 s by default). If nobody answers by then the panel is withdrawn and the dialog is left untouched.
...
It also registers a `PermissionRequest` hook, which lets an approval be answered from the web UI. When Claude is about to ask about a tool call, the hook publishes that approval as a structured question, waits a few seconds for an answer, and returns Claude's own allow/deny decision when one arrives. If nobody answers in time the hook produces no decision and Claude shows its normal prompt, so a terminal user is only delayed by that short window. Set `HERDR_CLAUDE_PERMISSION_WAIT_MS` in the agent's environment to change how long it waits. Approvals that are not answered in the web UI behave exactly as they did before the hook existed.
...
It also registers a `PermissionRequest` hook, which lets an approval be answered from the web UI. When Codex is about to ask about a command, file change, or permission, the hook publishes that approval as a structured question, waits a few seconds for an answer, and returns Codex's own allow/deny decision when one arrives. If nobody answers in time the hook produces no decision and Codex shows its normal prompt, so a terminal user is only delayed by that short window. Set `HERDR_CODEX_PERMISSION_WAIT_MS` in the agent's environment to change how long it waits. Approvals that are not answered in the web UI behave exactly as they did before the hook existed.

Like every Codex hook, it must be trusted before it runs. Codex asks for approval the first time it loads the hook; declining leaves the hook disabled and the integration otherwise working.
...
On Linux and macOS, the same install also enables the first-party Codex rollout
monitor on the Herdr server host. It reads the rollout JSONL associated with the
official Codex session id and reports structured activity diagnostics. It never
sends `continue`, presses Escape, interrupts a command, changes models, or
alters Herdr's `working`, `idle`, or `blocked` Agent status. Right-click an
Agent pane with an active diagnostic and choose **Inspect activity** to open a
closable card in the upper-right corner.

The monitor configuration is on the server host. Run the following there to
print its directory:

```bash
herdr plugin config-dir herdr.codex-rollout-monitor
```

Its `config.toml` defaults to:
...
`suspected_stalled_after_seconds` controls the warning threshold for an active
tool with no new rollout event. `silent_after_seconds` only changes the
intermediate command-running state. `notification.enabled` shows at most one
Herdr notification per stall episode; later rollout progress closes that
episode. `monitor.enabled` pauses monitoring without disabling the whole
plugin service. The monitor reloads this file while it runs. Native Windows
still installs the Codex hook but does not run this rollout-monitor service.
