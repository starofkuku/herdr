...
For supported agent panes, `Alt+V` can bridge an image from the local clipboard. The remote server first verifies the detected agent and its image-paste transport; only then does the local client read the clipboard and copy the image to a private temporary file on the remote host. Herdr pastes the absolute path for path-aware agents and an `@path` mention for Pi and Amp. If the pane is unsupported or has not been detected yet, Herdr forwards the original key without reading the clipboard.

| Remote agent | Image bridge |
| --- | --- |
| Claude Code, Codex, Cursor CLI, Cline, OMP, MastraCode, OpenCode, GitHub Copilot CLI, Kiro, Factory Droid, Grok, Hermes, Kilo Code, Qoder CLI, Maki, ZCode | Staged absolute image path |
| Pi, Amp | `@` mention of the staged absolute path |
| Gemini CLI | Not enabled: ordinary Herdr temporary paths are outside Gemini's allowed project directories |
| Kimi CLI | Not enabled: native image paste depends on Kimi's private prompt-cache token format |
| Devin CLI, Antigravity | Not enabled: no stable interactive CLI image-path contract is available |

Local clipboard reads are supported on macOS, Linux, and Herdr running inside WSL. Native Windows Herdr clients are not supported. This does not work when you SSH first and start Herdr on the server, because that process cannot access the local desktop clipboard.

On macOS, terminals normally consume `Command+V` themselves, while plain `Option+V` may be translated into a composed character instead of an Alt chord. A reliable local Mac binding is `keys.remote_image_paste = "ctrl+alt+v"`; detach and reconnect after changing it, then press Control+Option+V. Raw `Ctrl+V` is an agent-native clipboard shortcut in tools such as Codex, so sending it to an agent on a headless Linux host bypasses Herdr and may produce an X11 clipboard unavailable or connection-timeout error.
...
When your local and remote platforms match, Herdr can copy the current local binary for direct installs. For Homebrew, mise, and Nix installs, or when the platforms differ, this fork downloads the matching release asset for the current client version from `https://raw.githubusercontent.com/starofkuku/herdr/master/website/latest.json`.
