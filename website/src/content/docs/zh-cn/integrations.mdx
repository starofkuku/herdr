---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode 和 Grok Build 和 ZCode 安装 Herdr 集成。
---

Herdr 自动检测受支持的智能体。官方集成可以额外提供用于恢复的原生会话身份、生命周期状态上报,或两者兼有。

当你想要 Claude Code/Codex/Copilot/Devin 式钩子的智能体原生会话恢复、Pi/OMP/Kimi/OpenCode/Kilo/Hermes/Grok 式钩子或插件的直接生命周期上报,或两者都要时,使用集成。完整的状态权威模型见[智能体](/zh-cn/docs/agents/)。
...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、MastraCode、Grok Build | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
一些集成会上报智能体的原生会话引用。除非被 `[session] resume_agents_on_restore = false` 禁用,Herdr 会在服务器重启后使用官方会话引用恢复 Claude Code、Codex、Devin CLI、Droid、Kimi Code CLI、Qoder CLI、Cursor Agent CLI、GitHub Copilot CLI、Pi、OMP、Hermes Agent、OpenCode、Kilo Code CLI、MastraCode 和 Grok Build 的窗格。

原生会话恢复需要最新的 Herdr 集成: Pi 集成版本 `2`、OMP 版本 `3`、Claude Code 版本 `6`、Codex 版本 `7`、GitHub Copilot CLI 版本 `2`、Devin CLI 版本 `2`、Droid 版本 `2`、Kimi Code CLI 版本 `3`、Qoder CLI 版本 `2`、Cursor Agent CLI 版本 `1`、OpenCode 版本 `5`、Kilo Code CLI 版本 `1`、Hermes Agent 版本 `2`、MastraCode 版本 `1`、Grok Build 版本 `2`。用 `herdr integration status` 查看已安装版本。
...
ZCode 从自己的 `config.json` 读取 hook，而不是 `~/.claude/settings.json`——后者只作为只读的旧版兼容项列出。Herdr 在其中注册两个 hook：`SessionStart` 记录原生会话 ID（`sess_...`），使 Herdr 在服务端重启后可以用 `zcode --resume <id>` 恢复；`PermissionRequest` 让 Web UI 能在终端自行弹出提示前响应审批。agent 状态交由屏幕检测，与 Claude Code 集成一致。

只有用户级 hook 可用。ZCode 把项目级 hook 放在工作区信任审核之后，而该功能在其 CLI 和 TUI 中并未启用，因此安装在 `~/.zcode/cli/config.json` 之外的 hook 不会在那里执行。卸载会移除托管的 hook 脚本和 hook 条目，`config.json` 的其余内容保持不变。
...
若设置了 `GROK_HOME`，Herdr 改用 `$GROK_HOME/hooks/`。hooks 目录不存在时会自动创建。集成会上报生命周期状态和原生会话 id（`GROK_SESSION_ID` / 钩子载荷 `sessionId`），以便服务器重启后用 `grok --resume <id>` 恢复。子代理完成不会把根会话强制标为 idle；根会话的 `Stop` / `SessionEnd` 仍会。卸载会移除托管的钩子脚本和 JSON 文件。
...
安装 [`@juicesharp/rpiv-ask-user-question`](https://www.npmjs.com/package/@juicesharp/rpiv-ask-user-question) 插件后,该扩展还会把它的问卷映射到网页端。插件通过自身有文档的事件契约发布正在等待的问题,因此网页端可以提供 Agent 实际编写的选项,而不必读取屏幕。问题期间也会精确上报 `blocked`,不再依赖屏幕文本。

作答通过驱动插件自身的对话框送达:把光标移到所选行,然后按下 `Enter`。对话框始终保留在屏幕上,终端里同样可以作答,因此两条路径都可用,谁都不是唯一正解。

两个限制是刻意保留的。该对话框没有数字键(只响应方向键、`Enter`、`Esc`、`Tab`、`Space` 和少数控制键),所以选择靠移动光标再确认,而不是按数字。另外只映射单问题对话框:多问题对话框(标签页、复选框、每题的自定义回答)不存在忠实的按键序列,因此完全交给终端,而不是近似作答。自定义回答的输入同样留给终端。

Herdr 会在 `HERDR_PI_ASK_WAIT_MS`(默认 120 秒)内持续提示该问题。若届时无人作答,面板会被撤回,对话框保持原样。
...
它还会注册一个 `PermissionRequest` 钩子,使审批可以直接在网页端作答。当 Claude 即将就某个工具调用发起询问时,该钩子会把这个审批发布为结构化问题,等待数秒以获取作答,并在收到作答时返回 Claude 自身的 allow/deny 决定。如果无人及时作答,钩子不会输出任何决定,Claude 会显示它通常的提示,因此终端用户只会被这段短暂窗口延迟。可通过 Agent 环境中的 `HERDR_CLAUDE_PERMISSION_WAIT_MS` 调整等待时长。未在网页端作答的审批,其行为与没有这个钩子时完全一致。
...
它还会注册一个 `PermissionRequest` 钩子,使审批可以直接在网页端作答。当 Codex 即将就命令、文件变更或权限发起询问时,该钩子会把这个审批发布为结构化问题,等待数秒以获取作答,并在收到作答时返回 Codex 自身的 allow/deny 决定。如果无人及时作答,钩子不会输出任何决定,Codex 会显示它通常的提示,因此终端用户只会被这段短暂窗口延迟。可通过 Agent 环境中的 `HERDR_CODEX_PERMISSION_WAIT_MS` 调整等待时长。未在网页端作答的审批,其行为与没有这个钩子时完全一致。

与所有 Codex 钩子一样,它需要先被信任才能运行。Codex 会在首次加载该钩子时请求确认;拒绝后该钩子保持禁用,集成的其他部分照常工作。
...
在 Linux 和 macOS 上,同一个安装命令还会在 Herdr 服务器主机上启用第一方
Codex rollout 监控插件。它通过官方 Codex 会话 id 找到对应的 rollout JSONL,
并上报结构化活动诊断。它不会发送“继续”、按 Escape、中断命令、切换模型,
也不会改变 Herdr 的 `working`、`idle` 或 `blocked` 智能体状态。诊断存在时,
右击智能体窗格并选择 **Inspect activity**,即可在右上角打开可关闭的状态卡。

监控配置位于服务器主机。运行以下命令查看目录:
...
`suspected_stalled_after_seconds` 是活动工具长时间没有新 rollout 事件的警告
阈值;`silent_after_seconds` 只影响中间的命令静默运行状态。
`notification.enabled` 控制每轮疑似卡住时最多一次的通知,后续 rollout 进展会
结束这一轮。`monitor.enabled` 可以暂停监控,但不会禁用整个插件服务。配置文件
会在运行时重新加载。原生 Windows 仍会安装 Codex 钩子,但不会运行此监控服务。
