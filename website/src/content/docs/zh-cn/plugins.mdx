---
title: 插件
description: 编写带有清单动作、事件钩子、窗格和托管服务的本地 Herdr 插件。
---
...
动作 id、窗格 id、服务 id 和链接处理器 id 是插件内部的本地 id。它们可以使用
ASCII 字母、数字、冒号、下划线和连字符,但不能用点。每种 id 在插件内
必须唯一。当需要全局唯一名称时,Herdr 会把动作 id 限定为
`plugin.id.action` 的形式。

声明 `contexts = ["pane"]` 的动作也会出现在智能体窗格的右键菜单中。
当目标窗格被识别为智能体时,Herdr 会显示已启用的窗格动作;选择后会把
该窗格作为聚焦的插件上下文来调用动作。动作仍可通过 CLI 和快捷键调用。

用 `platforms = ["linux", "macos", "windows"]` 声明插件可以运行的平台。
构建命令、动作、事件钩子、窗格、服务和链接处理器也可以声明自己的 `platforms`;
...
`[[services]]` 用于监控器或桥接器这类长期运行的后台进程。Herdr 会在
服务器主机上为已启用插件的每项服务启动一个进程。服务会收到常规插件
环境变量,包括 `HERDR_PLUGIN_ROOT`、`HERDR_PLUGIN_CONFIG_DIR`、
`HERDR_PLUGIN_STATE_DIR`、`HERDR_BIN_PATH`、`HERDR_SOCKET_PATH`、
`HERDR_PLUGIN_ID` 和 `HERDR_PLUGIN_SERVICE_ID`。

`restart = "always"` 是默认值。服务退出或启动失败后,Herdr 会按一秒到
30 秒的指数退避重试;`restart = "never"` 则不重启。禁用或取消链接插件、
服务器实时交接以及服务器关闭时都会停止服务。服务不占用一次性插件命令
名额,标准输出和错误也不会记录为插件命令日志。

服务级 `platforms` 会覆盖插件顶层平台列表。`herdr plugin install` 的安装
预览会列出服务命令。服务是持续运行的普通受信任插件代码,审查插件时也应
检查这些命令。
