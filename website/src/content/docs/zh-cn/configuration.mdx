...
Windows Terminal 客户端（包括 WSL 配置文件）会为 `delivery = "terminal"` 使用 OSC 777，但外层终端必须包含 [microsoft/terminal#20012](https://github.com/microsoft/terminal/pull/20012)。截至 2026 年 7 月 15 日，Stable 1.24.11911.0 和 Preview 1.25.1912.0 均不受支持；请使用兼容的 Canary/开发构建，或等待 Windows Terminal 1.26 及更高版本正式发布，然后启用 `compatibility.allowOSC777`。设置方法和安全说明请参阅 [Windows 测试版](/docs/windows-beta/#windows-terminal-通知)。

## Bell 提醒

Bell 是一个独立且默认关闭的强提醒，用于相同的后台智能体完成和需要输入事件。它遵循通知延迟及活动标签页焦点抑制，但不要求开启 Toast 或声音。
...
Herdr 会通过前台客户端输出一个 ASCII BEL 字节，实际可见表现由外层终端和桌面环境决定：

- Windows Terminal 可高亮或闪烁任务栏图标。在配置文件默认设置中把 `bellStyle` 设为 `"taskbar"`（或 `"all"`）：
  ...
  ```
- 在 macOS 上,当 Ghostty 未聚焦时,`bell-features = attention` 会把 BEL 映射为一次 Dock 图标跳动。`attention` 默认开启,不需要 macOS 通知或辅助功能权限。修改 Ghostty 配置后按 `Command+Shift+,` 重新加载。在 `herdr --remote` 会话中,请在 Mac 本地客户端的配置里开启 `[ui.bell]`,因为 BEL 由该客户端发出。
- 在 Linux 上，任务栏高亮取决于终端模拟器和桌面环境是否支持视觉 Bell 或 urgency 提示。

tmux 等终端复用器可能拦截或抑制 BEL，因此也需要允许 Bell 传递到外层终端。Bell 只请求注意，不会抢占焦点。
...
如果你用 [codex-trace](https://github.com/xianchoujiduluo/codex-trace) 把 Agent 对话按轮次和工具调用阅读，Herdr 可以为窗格中的会话生成深链接：
...
设置后，右键 Agent 窗格会多出一项 **Copy session link**，它会把类似 `http://127.0.0.1:1422/pi/01a0acf2-81bb-76f7-9e48-763112a909d3` 的 URL 放进剪贴板。粘贴到浏览器即可打开该会话。

只有当窗格所属的 Agent 上报了转录路径、且文件名中能识别出会话 ID 时，这一项才会出现。支持 pi、Claude Code 和 Codex；其他 Agent 和普通 shell 没有可链接的会话。不设置 `url` 则完全隐藏该项。

复制走的是与鼠标选中相同的路径：终端支持时使用 OSC 52，否则调用平台剪贴板工具。正因如此，当 Herdr 运行在 SSH 或 WSL 上时，链接仍能到达你本机的剪贴板。
