...
当 Codex 界面显示精确的容量错误 `Selected model is at capacity. Please try a different model.` 时，Herdr 可以自动向报告错误的同一个 pane 发送 `继续` 和回车。
...
`max_retries` 默认是 `1`；设为 `0`（或 `enabled = false`）可关闭自动化，设为 `-1` 可无限重试。无限重试仍有 1 秒防抖，而且可能重复同一个渠道/模型的容量错误，因此只有在外部渠道会自动切换时才建议使用。
