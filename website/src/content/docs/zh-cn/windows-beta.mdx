...
Windows Terminal 可能为特定智能体支持图像粘贴路径,但 Herdr 自己的剪贴板图像读取器尚未接入原生 Windows 客户端。原生 Windows 的 `Alt+V` 图像粘贴仍属未验证。这与在 WSL 内运行 Herdr 不同: WSL 本地客户端可以通过 Windows PowerShell 使用仅限 Codex 的 `herdr --remote` 图像桥。

Kitty graphics 仍是实验性功能,尚未宣称支持 Windows。除非你专门在 Windows Terminal 中测试图像渲染,否则保持 `experimental.kitty_graphics = false`。

### Windows Terminal 通知

当外层 Windows Terminal 构建包含 OSC 777 桌面通知功能时，Herdr 可以从 Windows 原生会话和 WSL 配置文件发送智能体通知。这要求使用包含 [microsoft/terminal#20012](https://github.com/microsoft/terminal/pull/20012) 的 Canary/开发构建，或等待 Windows Terminal 1.26 及更高版本正式发布。截至 2026 年 7 月 15 日，Stable 1.24.11911.0 和 Preview 1.25.1912.0 均不支持 OSC 777，因此这些版本中的 `delivery = "terminal"` 不会产生 Toast 通知。

在受支持的 Windows Terminal 构建中，请在 `settings.json` 里为启动 Herdr 的配置文件启用兼容性设置：
...
受支持的 Windows Terminal 构建会在配置文件界面中显示 **允许 OSC 777 (Desktop Notification) 显示 toast 通知**。该设置默认关闭。如果找不到此选项，说明当前 Windows Terminal 构建不支持 OSC 777；仅更新到当前 Stable 或 Preview 版本仍不够。OSC 777 允许终端应用使用自定义文本创建 Windows 通知，因此只应在你信任所运行程序的配置文件中启用它。
