...
Herdr 可以通过 Windows Terminal 的 OSC 777 支持，从 Windows 原生会话和 WSL 配置文件发送智能体通知。在 Windows Terminal 的 `settings.json` 中，为启动 Herdr 的配置文件启用兼容性设置：
...
较新的 Windows Terminal 版本会在配置文件界面中显示 **允许 OSC 777 (Desktop Notification)显示 toast 通知**。如果找不到该选项，请更新 Windows Terminal。OSC 777 允许终端应用使用自定义文本创建 Windows 通知，因此只应在你信任所运行程序的配置文件中启用它。
