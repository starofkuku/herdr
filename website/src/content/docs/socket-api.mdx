...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.session`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.report_diagnostic`, `pane.clear_diagnostic`, `pane.report_interaction`, `pane.clear_interaction`, `pane.answer_interaction`, `pane.take_interaction_answer`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send`, `agent.rename`, `agent.focus`, `agent.start`, `agent.restart` |
...
Use `pane.report_diagnostic` for structured, short-lived inspection data that
must not take over Agent lifecycle state. `pane.get`, `pane.list`, `agent.get`,
and `agent.list` expose active diagnostics in their `diagnostics` array. Reports
are kept only in the server runtime and are not restored after a cold restart.

```json
{
  "id": "req_diagnostic",
  "method": "pane.report_diagnostic",
  "params": {
    "pane_id": "w1:p1",
    ...
    "summary": "The active tool has not produced a new event",
    "fields": [{"key":"tool","label":"Tool","value":"exec_command"}],
    "seq": 42,
    "ttl_ms": 5000,
    "updated_unix_ms": 1770000000000
  }
}
```

`source`, `diagnostic_id`, `state`, and field `key` are ASCII identifiers.
`title`, `summary`, field labels, and field values are normalized text. A pane
may retain up to eight diagnostics with at most 12 fields each. `ttl_ms` is
optional and uses the same 1–86400000 millisecond range as metadata. When a
report uses `seq`, later reports and `pane.clear_diagnostic` calls for the same
source and diagnostic id must use a strictly larger sequence number; an
unsequenced report cannot replace a sequenced record.
